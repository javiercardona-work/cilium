...
// TestTokenPathsOrder tests that token paths are tried in the correct order
func TestTokenPathsOrder(t *testing.T) {
	// Verify the token paths array is in the expected order
	...
	assert.Equal(t, expectedPaths, tokenPaths, "Token paths should be in priority order")
}
