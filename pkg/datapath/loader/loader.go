...
// it if necessary.
func reloadEndpoint(logger *slog.Logger, db *statedb.DB, devices statedb.Table[*tables.Device], rm *routeReconciler.DesiredRouteManager, ep datapath.Endpoint, lnc *datapath.LocalNodeConfiguration, spec *ebpf.CollectionSpec, tokenFD int) error {
...
func (l *loader) ReloadDatapath(ctx context.Context, ep datapath.Endpoint, lnc *datapath.LocalNodeConfiguration, stats *metrics.SpanStat) (string, error) {
	...
	stats.BpfLoadProg.Start()
	err = reloadEndpoint(l.logger, l.db, l.devices, l.routeManager, ep, lnc, spec, l.bpfTokenFD)
	stats.BpfLoadProg.End(err == nil)
	return hash, err
}
