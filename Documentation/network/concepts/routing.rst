...
``cilium_ipv4_over_ipv6_nodes`` BPF map. The agent repopulates this map from
``CiliumNode`` ``InternalIP`` state and clears stale entries on startup before
node-subscription replay.
